...
bool BGFXGpuProgram::set(const char* vshPath, const char* fshPath, const char* defines)
{
    ...
    const bgfx::Memory* memVsh = shaderc::compileShader(shaderc::ST_VERTEX, vshPath, defines, nullptr);
    const bgfx::Memory* memFsh = shaderc::compileShader(shaderc::ST_FRAGMENT, fshPath, defines, nullptr);

    if(!memVsh)
    {
        MV_WARN("Error while compiling vertex shader %s.", vshPath);
        return false;
    }

    if(!memFsh)
    {
        MV_WARN("Error while compiling fragment shader %s.", fshPath);
        return false;
    }
    ...
    if(!bgfx::isValid(_program))
    {
        MV_WARN("Error while creating bgfx program with shaders [%s], [%s], [%s].", vshPath, fshPath, defines);
        return false;
    }
...
void BGFXGpuProgram::getUniformsFromShader(bgfx::ShaderHandle shaderHandle)
{
    ...
    for (int i = 0; i < activeUniforms; ++i)
    {
        ...
        switch(info.type)
        {
            case bgfx::UniformType::Sampler: // Int, used for samplers only.
            ...
            case bgfx::UniformType::Mat4: // 4x4 matrix.
                ...
                break;
            default:
                MV_ERROR("Uniform type is unknown.");
        };
