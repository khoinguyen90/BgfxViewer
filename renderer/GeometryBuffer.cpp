...
bool GeometryBuffer::setRange(uint32_t start, uint32_t count)
{
    ...
    {
        MV_ERROR("Illegal buffer range : start + count > maxcount => (%d+%d > %d)", start, count, _elementCount);
        return false;
    }
...
void GeometryBuffer::set(const void* data, uint32_t count, uint32_t start)
{
    ...
    else
    {
        if(needResize)
        {
            MV_ERROR("Buffer overflow, static buffer is not allowed to resize.");
            return;
        }
    }
    ...
    if(data)
    {
        ...
        MV_ASSERT(memSize <= _memoryBuffer.getSize());
...
void* GeometryBuffer::lock(uint32_t start, uint32_t count)
{
    ...
    MV_ASSERT(_lockStart + _lockCount <= _elementCount);
    MV_ASSERT(_memoryBuffer.getSize() >= _elementSize * (_lockStart + _lockCount));

    if (_lockState != LOCK_NONE)
    {
        MV_ERROR("Buffer already locked.");
        return nullptr;
    }
...
void GeometryBuffer::unLock()
{
    ...
    {
        MV_ERROR("Buffer was not locked.");
    }
}
