    ...
    _handle(BGFX_INVALID_HANDLE)
{
    ...
    switch(type)
    {
        case UniformType::UT_SAMPLER: // Int, used for samplers only.
            bgfxUniformType =  bgfx::UniformType::Sampler;
            break;
        ...
        case UniformType::UT_MATRIX4: // 4x4 matrix.
            ...
            break;
        default:
            MV_ERROR("BGFXGpuProgram: Uniform type unknown.");
    };

    _num = num;
    _handle = bgfx::createUniform(name, bgfxUniformType, num);
    MV_ASSERT(bgfx::isValid(_handle));
}
...
void BGFXUniform::setValue(int value)
{
    ...
    MV_ERROR("BGFXUniform::setValue : int reserved for samplers.");
